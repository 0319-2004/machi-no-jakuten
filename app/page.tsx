...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/data/itabashi-shelters.geojson")
      .then((response) => {
  ...
  useEffect(() => {
    ...
    setTimeIndex(0);
    setPlaying(false);
    fetch(`/api/shinsui?kind=breakpoints&lat=${place.lat}&lon=${place.lng}`, {
  ...
  useEffect(() => {
    ...
    setHydroFreshness(null);
    setTimeIndex(0);
    fetch(`/api/shinsui?kind=hydrograph&lat=${place.lat}&lon=${place.lng}&bpid=${selectedBreakpoint.ID}`, {
      signal: controller.signal,
      cache: "no-store",
    })
      .then(async (response) => {
